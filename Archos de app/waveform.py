...
- Librería: escaneo de carpetas, base SQLite, filtros (fabricante/marca/modelo/año/engine),
  previsualización de waveform al seleccionar, cargar al editor, editar metadata, borrar múltiple
...
def write_waveform_to_oto(base_path: str, out_path: str, data_720: Dict[str, List[int]],
    ...
    for col_idx in range(1, len(CHANNELS) + 1):
        ...
        if not channel_enabled.get(ch, True):
            # si está deshabilitado, no lo forzamos a modificar
            continue
...
class WaveDB:
    ...
    def _init(self):
        ...
        """)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_filters ON waveforms(fabricante, marca, modelo, year, engine)")
        self.conn.commit()

    def upsert(self, fabricante: str, marca: str, modelo: str, year: str, engine: str, file_path: str):
        file_name = os.path.basename(file_path)
        ts = time.time()
        cur = self.conn.cursor()
        cur.execute("""
        INSERT INTO waveforms(fabricante, marca, modelo, year, engine, file_path, file_name, updated_ts)
        VALUES(?,?,?,?,?,?,?,?)
        ...
        """, (fabricante, marca, modelo, year, engine, file_path, file_name, ts))
        self.conn.commit()
    ...
    def update_record(self, rec_id: int, fabricante: str, marca: str, modelo: str, year: str, engine: str):
        cur = self.conn.cursor()
        cur.execute("""
        UPDATE waveforms
        SET fabricante=?, marca=?, modelo=?, year=?, engine=?, updated_ts=?
        WHERE id=?
        """, (fabricante, marca, modelo, year, engine, time.time(), rec_id))
        self.conn.commit()

    def list_distinct(self, field: str, where: Dict[str, str]) -> List[str]:
        allowed = {"fabricante", "marca", "modelo", "year", "engine"}
    ...
    def query(self, fabricante: str = "", marca: str = "", modelo: str = "", year: str = "", engine: str = "",
        ...
        if text.strip():
            t = f"%{text.strip()}%"
            clauses.append("(file_name LIKE ? OR file_path LIKE ? OR modelo LIKE ? OR engine LIKE ?)")
            params.extend([t, t, t, t])
        wsql = ("WHERE " + " AND ".join(clauses)) if clauses else ""
        cur = self.conn.cursor()
        cur.execute(f"SELECT * FROM waveforms {wsql} ORDER BY fabricante, marca, modelo, year, engine, file_name", params)
        out = []
...
class ThemeManager:
    ...
    def apply(self):
        ...
        # Usa 'clam' para que configuremos colores de ttk consistentemente
        try:
...
class PlotAndToolsTab(ttk.Frame):
    ...
    def _build_ui(self):
        # Layout: izquierda tools, derecha plot
    ...
    def _on_toggle_channels(self):
        ...
        # Si el canal seleccionado se deshabilitó, selecciona otro
    ...
    def _on_plot_hover(self, event):
        ...
        # valores por canal (solo habilitados) en este grado
        parts = [f"Grado: {deg}"]
        for ch in CHANNELS:
    ...
    def _on_plot_click(self, event):
        # Click izquierdo: toggle bit del canal seleccionado en el grado cercano
        if event is None or event.inaxes != self.ax or event.xdata is None:
            return
        if event.button != 1:
            return
    ...
    def redraw_plot(self):
        ...
        # cursor line se recrea en hover
        self.cursor_line = None
        self.canvas.draw_idle()

    # ---- Integración con menú (MainApp) ----
...
class ImageTab(ttk.Frame):
    ...
    def load_image(self):
        ...
        # Resize conservador para pantalla
...
def guess_meta_from_path(root: str, file_path: str) -> Tuple[str, str, str, str, str]:
    """
    Interpreta estructura:
        root/fabricante/marca/modelo/year/engine/file.oto

    Si faltan niveles, deja en blanco.
    """
    try:
    ...
    # Esperado: 6 partes mínimo (fab, marca, modelo, year, engine, file)
    fabricante = parts[0] if len(parts) >= 2 else ""
    marca = parts[1] if len(parts) >= 3 else ""
    modelo = parts[2] if len(parts) >= 4 else ""
    year = parts[3] if len(parts) >= 5 else ""
    engine = parts[4] if len(parts) >= 6 else ""
    return fabricante, marca, modelo, year, engine
...
class LibraryTab(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        b1, self.cb_fab = combo("Fabricante", self.fab_var, 18)
        b2, self.cb_marca = combo("Marca", self.marca_var, 18)
        b3, self.cb_modelo = combo("Modelo", self.modelo_var, 20)
        b4, self.cb_year = combo("Año", self.year_var, 10)
        ...
        for cb in [self.cb_fab, self.cb_marca, self.cb_modelo, self.cb_year, self.cb_engine]:
        ...
        # Treeview list
        lft = ttk.Labelframe(left, text="Resultados (multi-selección para borrar)")
        lft.pack(fill="both", expand=True)

        cols = ("fabricante", "marca", "modelo", "year", "engine", "file")
        self.tree = ttk.Treeview(lft, columns=cols, show="headings", selectmode="extended")
        for c in cols:
            self.tree.heading(c, text=c)
        self.tree.column("fabricante", width=110, anchor="w")
        self.tree.column("marca", width=90, anchor="w")
        self.tree.column("modelo", width=180, anchor="w")
        self.tree.column("year", width=60, anchor="center")
        self.tree.column("engine", width=110, anchor="w")
    ...
    def on_filter_change(self):
        # refresca combos dependientes y lista
        self.refresh_filters()
        self.refresh_list()
    ...
    def refresh_filters(self):
        ...
        # modelo (depende de fabricante+marca)
        dep = {}
        if self.fab_var.get():
        ...
        # engine (depende fab+marca+modelo+year)
        dep3 = dict(dep2)
    ...
    def refresh_list(self):
        ...
        self.records = self.db.query(fabricante=fab, marca=marca, modelo=modelo, year=year, engine=engine, text=text)
        ...
        for rec in self.records:
            self.tree.insert("", "end", iid=str(rec.id),
                             values=(rec.fabricante, rec.marca, rec.modelo, rec.year, rec.engine, rec.file_name))
        ...
        try:
            for dirpath, _, filenames in os.walk(root):
                for fn in filenames:
                    ...
                    fab, marca, modelo, year, engine = guess_meta_from_path(root, fp)

                    # si algo viene vacío, lo dejamos y luego puedes editar
                    self.db.upsert(fab, marca, modelo, year, engine, fp)
                    n += 1
        except Exception as e:
    ...
    def on_select_row(self, event=None):
        ...
        # Para preview usamos el primer seleccionado
        try:
            rid = int(sel[0])
        except Exception:
            return
    ...
    def draw_preview(self, path: str):
        """
        Preview robusto: auto-ajusta ylim para que NO se recorten ondas apiladas.
        """
        c = self.theme.colors()
        ...
        # Usa habilitación del editor (si te sirve como referencia visual)
        ...
        # FIX: límites Y para no recortar
        if plotted > 0:
            max_offset = (plotted - 1) * offset_step
    ...
    def load_selected_to_editor(self):
        ...
        try:
            ...
            self.app.notebook.select(self.app.plot_tab)  # cambia a editor
    ...
    def edit_selected(self):
        ...
        v_fab = row("Fabricante", rec.fabricante)
        v_mar = row("Marca", rec.marca)
        v_mod = row("Modelo", rec.modelo)
        v_year = row("Año", rec.year)
        v_eng = row("Engine", rec.engine)

        btn = ttk.Frame(frm)
        btn.pack(fill="x", pady=(10, 0))
        ttk.Button(btn, text="Cancelar", command=win.destroy).pack(side="right")
        ttk.Button(btn, text="Guardar", command=lambda: self._save_edit(win, rec.id, v_fab, v_mar, v_mod, v_year, v_eng)).pack(side="right", padx=6)

    def _save_edit(self, win, rid: int, v_fab, v_mar, v_mod, v_year, v_eng):
...
class SettingsTab(ttk.Frame):
    ...
    def apply_theme(self):
        self.theme.apply()
        # Redibujar plots con nuevo fondo
        self.app.plot_tab.redraw_plot()
        self.app.library_tab.draw_preview(self.app.library_tab.selected_rec.file_path) if self.app.library_tab.selected_rec else None
